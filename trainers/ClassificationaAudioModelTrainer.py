...
from conts import Paths, Comet
from data import DataGrabber


class ClassAudioTeacher:
    ...
    def __init__(self, num_epochs=100, batch_size=32, count_first_layer=128, count_second_layer=64,
                 ...
                 comet_project_name="RawAudio", checkpoint_name=None, cuda_support=False, vst_parameter=None,
                 convolutional_lenet=False):
    ...
    def createConvModel(self, train_shape, output_count):
        ...
        lenet_5_model.compile(loss='categorical_crossentropy', metrics=['accuracy'], optimizer='adam')
    ...
    def prepare_data_and_train(self, features_temp, trained_model=None):
        ...
        feature_array = np.asarray(features_panda.feature.tolist())[0]
        value_array = np.asarray(features_panda.class_label.tolist())[0]
        del features_panda
        value_array = value_array[0:len(value_array),0:1]
        # for i, value in enumerate(value_array):
        #     value_array[i] = value[0]
        value_array.reshape(len(value_array))
        le = LabelEncoder()
        ...
        if trained_model is None:
            ...
            if self.convolutional_lenet:
                x_train = x_train.reshape(-1, 110250, 1)
                x_test = x_test.reshape(-1, 110250, 1)

                train_shape = x_train[1].shape

                model = self.createConvModel(train_shape, out_shape)
            else:
                model = self.createModel1D(train_shape, out_shape)
        else:
            model = trained_model
...
teacher = ClassAudioTeacher(num_epochs=1000, checkpoint_name="denseaudiocat_0oscshape_raw_audio")
